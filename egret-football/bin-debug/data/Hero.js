...
var Hero = (function () {
    ...
    function Hero(target) {
        ...
        this.returnNodePre = 0.4; //返回时切换状态所在时所占百分比
        this.returnNodeY = 150; //在变化的节点时的Y轴位置
        this.returnType = 0; //返回返回路径的样式
        this.returnX = -75; //返回目标位置的X坐标
        this.returnY = 385; //返回目标位置的Y坐标
        this.returnSpeed = 10; //足球返回的速度
        this.isCommon = false; //标示是否是随便踢效果
        this.attackIndex = 0; //标示攻击特效的种类
        this.targer = target;
    ...
    p.flash = function () {
        ...
        if (GameData.ballState == "shoot") {
            ...
            else if (pre > 0.1 + this.returnNodePre) {
